# Abstract superclass.
# A type is a relation in relational terminology; we restrict the kinds of relations we allow though
# to a snowflake-style reporting schema
class Factstar::Schema::Type
  ...
  def initialize(schema, name, options={}, &block)
    ...
    # bit MSP-specific this, but a media type for types corresponding to MSP::Resource::Base media types identified by their id:
  ...
  def dimension(name, type, options={}, &block)
    raise Factstar::InvalidSchemaDefinition, "Dimension #{name} already registered" if @dimensions_by_name[name.to_s]
    type = @schema[type] || raise(InvalidSchemaDefinition, "Type #{type} not found in schema yet") if type.is_a?(Symbol)
    @dimensions_by_name[name.to_s] = Factstar::Schema::Dimension.new(name, type, options, &block)
  end
  ...
  def columns_and_table_with_joins_for_dimension_paths(paths, path_to_self=[self.name], paths_forced_not_null=[], range=nil, temp_table_context=nil)
    raise Factstar::NotImplementedException unless table
